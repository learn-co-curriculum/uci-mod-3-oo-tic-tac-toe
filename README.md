...
`01_tic_tac_toe_spec.rb` sets tests for our helper methods within `TicTacToe`.
...
entering 1-9. Our program will then fill out the appropriate position on the
board with the player's token.
...
`TicTacToe` in `lib/tic_tac_toe.rb`. Without that file defining a `TicTacToe`
class, everything will break.
...
equal to a nested array filled with the index values for the various winning
...
**Tip:** The next bunch of methods we will be describing have already been
defined in previous labs. You can copy your code from those labs, paste them in
this one, and tweak them slightly to work with the object oriented approach to
pass the tests.
...
perspective to the array's — accounting for the fact that `@board[5]` is _not_
where the user intended to place their token.
...
individual methods that you've already built.
...
present in the board and return the winning combination indexes as an array if
there is a win. Use your `WIN_COMBINATIONS` constant in this method.
